...
class TagFilter(Filter):
    ...
    def __init__(
        self, tags: set[str] = set(), logic: Logic = Logic.NONE, mode: Mode = Mode.NONE
    ):
        self.tags = tags
        self.logic = logic
        self.mode = mode
    ...
    def apply(self, dataset):
        if (
            not self.tags
            or self.logic == TagFilter.Logic.NONE
            or self.mode == TagFilter.Mode.NONE
        ):
            return dataset
...
class TagScoreFilter(Filter):
    ...
    def apply(self, dataset):
        ...
        paths_remove = {
            path
            for path, scores in self.scores.items()
            if (scores.get(self.tag) or 0) > self.threshold
        }
